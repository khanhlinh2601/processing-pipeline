...
export interface LineageNode {
  ...
  parentId: string | null;  // parent node ID for hierarchy
  jobId: string;
...
export class LineageNodeRepository {
  ...
  constructor(private configService: ConfigService) {
    ...
    this.tableName = this.configService.get<string>('DYNAMODB_LINEAGE_NODE_TABLE') || 'document-lineage-node-dev';
  }

  async createNode(node: Omit<LineageNode, 'createdAt' | 'updatedAt'>): Promise<LineageNode> {
    try {
  ...
  async findByNodeType(nodeType: string): Promise<LineageNode[]> {
    try {
      this.logger.log(`Finding lineage nodes of type ${nodeType}`);
      
      // Using scan with filter since we don't have a specific index for nodeType
      // In a production environment, consider creating a GSI for nodeType for better performance
      const command = new ScanCommand({
        TableName: this.tableName,
        FilterExpression: 'nodeType = :nodeType',
        ExpressionAttributeValues: {
          ':nodeType': nodeType,
        },
      });
      ...
      this.logger.error(`Failed to find lineage nodes by type: ${error.message}`);
      throw error;
    }
  }
  ...
  async bulkCreate(nodes: Array<Omit<LineageNode, | 'createdAt' | 'updatedAt'>>): Promise<LineageNode[]> {
    try {
